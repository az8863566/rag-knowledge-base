...
class SiliconFlowClient:
    ...
    def _request_with_retry(
        self, url: str, payload: dict, max_retries: int = 3, timeout: int = 60
    ) -> dict:
        """带重试的请求"""
        last_error = None
    ...
    def chat_completions(
        ...
        Raises:
            SiliconFlowError: API 调用失败时抛出
        """
        if not messages:
            raise SiliconFlowError("消息列表不能为空")
        ...
        logger.debug(f"LLM 对话: model={model}, messages={len(messages)}")
        data = self._request_with_retry(url, payload, timeout=120)

        # 验证响应格式
        choices = data.get("choices", [])
        if not choices:
            raise SiliconFlowError("LLM 响应格式错误: 缺少 'choices' 字段")

        message = choices[0].get("message", {})
        content = message.get("content", "")
        if not content:
            raise SiliconFlowError("LLM 响应格式错误: 缺少回复内容")

        logger.info(f"LLM 对话完成: 回复长度={len(content)}")
        return content
    ...
    def chat_completions_stream(
        ...
        logger.debug(f"LLM 流式对话: model={model}, messages={len(messages)}")
        ...
        try:
            ...
            for line in resp.iter_lines():
                ...
                try:
                    ...
                    logger.warning(f"解析流式响应失败: {data_str}")
                    continue

            yield {"type": "done"}
            logger.info(f"LLM 流式对话完成: thinking={thinking_chars}字, answer={answer_chars}字")

        except requests.exceptions.Timeout:
            raise SiliconFlowError("LLM 请求超时")
        except requests.exceptions.ConnectionError as e:
            raise SiliconFlowError(f"连接错误: {e}")
        except requests.exceptions.RequestException as e:
            raise SiliconFlowError(f"请求错误: {e}")
