...
class Config:
    ...
    def set_similarity_threshold(self, value: float) -> None:
        ...
        self._runtime_threshold = float(value)
        logger.info(f"相似度阈值已更新为: {self._runtime_threshold}")

    def reset_similarity_threshold(self) -> None:
        """恢复使用配置文件中的相似度阈值"""
        self._runtime_threshold = None
        logger.info("相似度阈值已恢复为配置文件值")

    @property
    def is_threshold_runtime(self) -> bool:
        """阈值是否为运行时设置"""
        return self._runtime_threshold is not None
    ...
    def chat_model(self) -> str:
        return self.get("chat.model", "THUDM/GLM-4.1V-9B-Thinking")

    @property
    def chat_system_prompt(self) -> str:
        return self.get(
            "chat.system_prompt",
            "你是一个知识库问答助手。请根据以下参考文档回答用户问题。如果参考文档中没有相关信息，请如实说明。\n\n参考文档：\n{context}"
        )

    @property
    def chat_max_tokens(self) -> int:
        return self.get("chat.max_tokens", 2048)
